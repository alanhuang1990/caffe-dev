...
void MaskConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // Configure the kernel size, padding, stride, and inputs.
	...
	MaskConvolutionParameter mask_conv_param = this->layer_param_.mask_convolution_param();
	string mask_file_name = mask_conv_param.mask_rules_file_path();

	// parsing mask file
	FILE * mask_fid = fopen(mask_file_name.c_str(),"r");
	int buf_int=0;
	vector<int> v_int_buf;
	while(fscanf(mask_fid,"%d",&buf_int) == 1)
	{
		v_int_buf.push_back(buf_int);
	}
	CHECK(v_int_buf.size() == this->channels_ * this->num_output_) << "the rules in mask file does not "
			<<"match net structure";
	int idx = 0;
	for (int i=0;i< this->num_output_; i++)
	{
		...
		{
			v_row.push_back(v_int_buf[idx++]);
		}
		this->Masks_.push_back(v_row);
	}
	fclose(mask_fid);
